...
`reasoning_content` / `reasoning` フィールドを返すサーバがある（DeepSeek-R1系など）。
これを見つけたら `reasoning-delta` イベントとして別に流し、本文と混ぜない。
画面では折りたたんで表示する。
...
そのため実行の直前に、次のいずれかなら判定を1度だけ行い、結果を保存する。

- `capabilities.tools` が `auto`
- 保存された `probedModel` が、今回使うモデルと違う（判定はモデルごとに変わるため）
