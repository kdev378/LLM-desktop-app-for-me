...
- **サーバが返した順序を保つ。** 並べ替えない。
  読み込み中のモデルが先頭に来るなど、順序に意味があるサーバがある。
  表示のための並べ替えは、表示する側で行う。
- サーバが文脈長を返していれば拾う。名前はサーバごとに違うので、
  `max_model_len`（vLLM）/ `max_context_length` / `context_length` / `n_ctx` /
  `context_window` を順に見る。**どれも無ければ「不明」とする。推測しない。**

### LM Studio 固有の口から補う

OpenAI互換の `/models` は文脈長を返さない。LM Studio は独自に
`{origin}/api/v0/models` を持ち、そこに `max_context_length` /
`loaded_context_length` / `state` が入っている。

- `/models` を取った後に、この口へ best-effort で1回問い合わせる（上限3秒）。
- 取れたら `contextTokens`（**読み込み中の長さを優先**）と `state` を重ねる。
  `/models` 側が既に文脈長を持っていれば、そちらを優先する。
- **取れなくても異常ではない。** LM Studio でなければ 404 が返るだけ。黙って諦める。
- `state` が `loaded` でないモデルを判定に使うときは、その旨を表示する。
  読み込まれていないモデルは、判定と実行で挙動が変わりうる。

**この口の応答の形は、実物で検証できていない。** 実装は「あれば読む」形にしてあるので、
形が違っても「文脈長は不明」に落ちるだけで、壊れない。

### モデル未指定のときの選び方

一覧の先頭をそのまま使わない。**LM Studio のように、埋め込みモデルが
一覧に混ざる環境がある。** それを掴むと chat が 400 で失敗し、
利用者からは「何も判定されない」ようにしか見えない。

- 名前に `embed` / `rerank` / `whisper` / `tts` / `bge` / `clip` / `moderation` を
  含むものは会話用ではないとみなし、飛ばす。
- 会話用が1つも無ければ、先頭をそのまま使う（何も選ばないより良い）。
  ただし「専用モデルに見える」と警告する。
- **どのモデルを選んだかを必ず表示する。** 成功しても失敗しても出す。
  出さないと、失敗したとき何を試したのか分からない。

### 判定結果は「何が起きたか」まで出す

「ツール呼び出しに対応していない」には理由が複数ある。区別しないと直しようがない。

| 観測 | 判定 | 表示すること |
|---|---|---|
| `tool_calls` が来た | `native` | どのモデルで確認したか |
| 400 で `tools` 引数を拒否された | `prompted` | サーバが引数そのものを拒否したこと |
| 200 で本文だけ返った | `prompted` | **サーバは受け付けたが、モデルが呼ばなかった**こと。多くはモデルの配布物にツール用テンプレートが無い。同じモデルでも配布元で変わる |
| 応答が空・エラー | `none` | サーバの応答本文（先頭300文字）。`none` は**非対応ではなく「判定できなかった」**。実行時は `both` で試す |

トークン数は「報告するかどうか」だけでなく、**その判定で実際に何トークンだったか**を出す。
...
| 対象 | 既定 | 超えたら |
|---|---|---|
| モデル一覧（`/models`） | 10秒 | `unreachable` |
| 生成: リクエスト〜最初のトークン | `endpoint.timeoutMs`（既定300秒） | `unreachable` |
| 生成: トークン間 | 120秒（1トークンごとに数え直す） | `incompatible` 扱いで中断 |
| 全体 | 上限なし | 利用者の停止に任せる |

**生成では、応答ヘッダまでと最初のトークンまでを分けない。**
ローカルのサーバは、モデルを読み込み終わるまで応答ヘッダすら返さないことがある。
そこに短い「接続確立」の上限を当てると、読み込み待ちが「繋がらない」として切られる。
待たされる理由の無いモデル一覧だけ、短い上限で見る。

既定が300秒なのは、想定する環境が「手元の機械で数B級のモデルを動かす」ためである。
モデルの読み込みと、長い入力の処理（8個のツール定義＋指示ファイル＋履歴）が
最初のトークンより先に来る。

超えたときは、実際の上限の秒数と**延ばし方**（`akari config endpoints set --timeout`）を
メッセージに入れる。利用者が次に何をすればよいか分からないまま終わらせない。

待ち時間は次の3つで変えられる。強い順に、`--timeout`（その1回だけ）、
`AKARI_TIMEOUT`（環境変数）、`endpoint.timeoutMs`（設定）。

## 思考量の指定

Qwen3 系のように思考を出すモデルでは、思考が長いと待ち時間とトークンを食う。
文脈長の小さいモデルでは、思考だけで入りきらなくなる。

利用者から見える指定は1つ（`auto` / `off` / `low` / `medium` / `high`）。
送り先は2つに分かれる。**同じ意味の口が2種類あるからで、両方は送らない。**
...
`off` だけ別の口を使うのは、思考を出すか出さないかがチャットテンプレート側の
切り替えで、量の指定とは別の仕組みだから。

### 受け付けないサーバへの対応

`reasoning_effort` も `chat_template_kwargs` も、理解せず 400 を返すサーバがある。
`stream_options` と同じ扱いにする。**任意パラメータは、拒否されたら1つ落として送り直す。**

- 400/422 の本文にパラメータ名が出ていて、かつ**実際に送っていた**ものだけを落とす
- 送っていないものは落とせない（名前を含む 400 で無限に送り直さない）
- 落としたことは `notice` として呼び出し側へ渡す

**黙って落とさない。** 指定が効いていないのに効いたように見せると、
「なぜ思考が止まらないのか」が分からなくなる。

対象の任意パラメータ: `stream_options` / `reasoning_effort` / `chat_template_kwargs`

### 未検証

**どのサーバがどちらの口を受け付けるかは実機で確認していない。**
`reasoning_effort` は OpenAI 系の名前で、LM Studio が一部のモデルで受ける。
`chat_template_kwargs` は llama.cpp / vLLM 系で Qwen3 の切り替えに使われる。
外れていても、上の落として送り直す仕組みで失敗はしない（指定が効かないだけ）。
...
判定結果 `none` は「ツールに対応していない」ではなく、**「判定できなかった」**を意味する。
到達できなかった、応答が空だった、判定の生成でエラーが出た、のいずれかである。
非対応と分かった場合は `prompted` になる。

### 両対応（both）

`none`（判定できなかった）ときに実行を断らない。断ると、ツールを呼べるモデルまで
使えなくなる。代わりに **`both`**（両対応）で実行する。

`both` では次の両方を行う。

- リクエストに `tools` を載せる（対応していれば `tool_calls` が返る）
- システムメッセージに `akari-tool` ブロックの書き方も入れる

応答の解釈は**先に来たほうを採る**。

1. `tool_calls` が1つでも返っていれば、それを実行する。本文のブロックは**見ない**。
2. `tool_calls` が無いときだけ、本文から `akari-tool` ブロックを取り出す。

同じ応答に両方が入っていても二重に実行しない。
モデルには「どちらか片方を使う」と明示する。

ツール結果は、**呼ばれた形に合わせて返す**。

| 呼ばれ方 | 返し方 |
|---|---|
| `tool_calls` | `role: "tool"`（`tool_call_id` 付き） |
| `akari-tool` ブロック | `role: "user"` の `[akari-tool-result]` |

混ぜてはいけない。`tool_calls` の無い会話へ `role: "tool"` を入れると、
多くのサーバが 400 を返す。

`both` で動いていることは「対応が確認できていない」として常時表示する。
判定できたふりをしない。

### 渡し方の一覧

| 方式 | `tools` を送る | ブロックの説明を入れる | ブロックを解釈する |
|---|---|---|---|
| `native` | ○ | × | × |
| `prompted` | × | ○ | ○ |
| `both` | ○ | ○ | `tool_calls` が無いときだけ |

判定を無視して方式を決めたいときは、`--tools-mode`（CLI）、`AKARI_TOOLS_MODE`（環境変数）、
`agent.toolsMode`（設定）で指定できる。既定は `auto`（判定に従う）。
明示されている場合、判定の呼び出しそのものを省く。
