...
```sh
bash tools/verify-local-llm.sh http://localhost:11434/v1 gemma3n:e4b
```
...
- 「ツール呼び出し: 対応」→ そのまま使える
- 「呼ばれず本文だけが返りました。代替方式（prompted）になります」
  → 自動で代替方式に切り替わる。Gemma 系ではこちらになる見込み
- 「判定できていません」→ 5番も動かない。モデルかサーバ側の問題
