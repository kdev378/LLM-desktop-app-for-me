...
対象は「10秒版」（`--tools-mode` も `--timeout` も無い版）。
新しい版でだけ試せるものは最後の「新版でだけ試せること」にまとめてある。
...
### 11-1. 判定できないモデルでも道具を使う

```sh
akari run --tools-mode both "README を読んで要約して" -m <モデルID>
```

**期待**: `ツール: 8種 / 両対応` と出て、判定を待たずに実行が始まる。
モデルが `tool_calls` を返せばそれを使い、返さなければ本文のブロックを使う。

```sh
akari run --tools-mode native   "…"    # 関数呼び出しだけ
akari run --tools-mode prompted "…"    # 本文のブロックだけ
```

10秒版で `判定できていません` になったモデルは、新版なら `both` で動く。
