...
| 2026-09-05 | 実装順序をエージェント先行に変更（デスクトップは P6 へ） | 価値の中心が「手を動かす」ことで、外部ツールから使えるのが最短の実用化。画面は中身ができてから被せるほうが手戻りが少ない | 順序を戻すだけ |
...
- 未実装: ハーネスAPI（P2）、MCP（P3）、記憶（P4）、Web（P5）、デスクトップ（P6以降）
- 次にやること: 実機4種での疎通確認 → **P2（ハーネスAPI）**。
  外部のマルチエージェントツールから使えるようになるのが最短の実用化のため
